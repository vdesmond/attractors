...
def test_attractors_des(attr, attractor_obj_des):
    ...
    for i in range(len(attrparams["params"])):
        assert (
            getattr(attractor_obj_des, attrparams["params"][i]) == attrparams["default_params"][i]
        )
...
def test_attractors_des_rk2(attr, attractor_obj_des_rk2):
    ...
    for i in range(len(attrparams["params"])):
        assert (
            getattr(attractor_obj_des_rk2, attrparams["params"][i]) == attrparams["default_params"][i]
        )
